...
def update_category_score():
    try:
        ...
        database.user_to_category_browsing(
            user_email, category_name, duration, datetime.now())
    ...
    try:
        ...
        if email:
            data = database.get_blogs_by_likes(
                email, category_name=category, page=page)
            return jsonify(data)
        else:
            data = database.get_blogs_by_likes(
                email, category_name=category, page=page)
...
def get_recent_articles_by_category():
    try:
        ...
        data = database.get_recent_blogs(
            email, category_name=category, page=page)
        return jsonify(data)
    except Exception as e:
        return jsonify(error=str(e)), 500
...
def get_hot_articles_by_category():
    try:
        ...
        page = int(args.get('page'))
        email = args.get('email')
        data = database.get_hot_blogs(
            email=email, category_name=category, page=page)
        return jsonify(data)
    except Exception as e:
        return jsonify(error=str(e)), 500
